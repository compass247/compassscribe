...
> CMS tại cms.compassscribe.com, lead API nguyên vẹn. Tài liệu này giữ lại làm tham chiếu
...
Hướng dẫn từng bước đưa CMS lên production sau khi PR [#4](https://github.com/compass247/compassscribe/pull/4) được merge. Mỗi bước đều có cách kiểm chứng. Thứ tự quan trọng — làm tuần tự.
...
- `CMS_BASE` = `https://cms.compassscribe.com`
- `SITE_URL` = `https://compassscribe.com`
...
> Đặt nhanh: `gh variable set CMS_BASE --body "https://cms.compassscribe.com"`.
...
```bash
aws ecs list-services --cluster cmas                 # thấy cmas-cms
curl -s https://cms.compassscribe.com/server/health     # {"status":"ok"}
```
...
```
Đăng nhập `https://cms.compassscribe.com` (admin@compassscribe.com). **Đổi mật khẩu ngay.**

Tạo schema (chạy từ máy có quyền, trỏ vào CMS prod):
```bash
TOKEN=$(curl -s -X POST https://cms.compassscribe.com/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"admin@compassscribe.com","password":"<new-pass>"}' | jq -r .data.access_token)

DIRECTUS_URL=https://cms.compassscribe.com DIRECTUS_TOKEN=$TOKEN \
...
```bash
curl -s "https://cms.compassscribe.com/items/homepage?fields=translations.hero" | jq '.data.translations|length'
```
...
- Operation **Webhook**: `POST https://compassscribe.com/api/revalidate?secret=<REVALIDATE_SECRET>`, body `{"collection":"{{$trigger.collection}}","slugs":["{{$trigger.payload.slug}}"]}`.

Lấy secret: `aws secretsmanager get-secret-value --secret-id cmas-cms --query SecretString --output text | jq -r .REVALIDATE_SECRET`.
...
**Kiểm chứng:**
```bash
curl -s -o /dev/null -w "%{http_code} -> %{redirect_url}\n" https://compassscribe.com/   # 307 -> /vi
curl -s https://compassscribe.com/vi | grep -o "<title>[^<]*"
curl -s -A "Googlebot" https://compassscribe.com/vi/blog/<slug> | grep -o "<h1>[^<]*"   # HTML thật
curl -s https://compassscribe.com/sitemap.xml | grep -c "<loc>"
```
...
- Submit `https://compassscribe.com/sitemap.xml` lên Google Search Console.
