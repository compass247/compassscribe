...
> Compassscribe — từ khái niệm cơ bản nhất đến khi sản phẩm chạy thật trên Internet.
>
...
- Mỗi chương có cùng một khuôn:
  ...
  3. **📁 Trong dự án Compassscribe của bạn** — file/lệnh/kết quả thật bạn đã thấy.
...
| Nhà hàng | Website Compassscribe |
|---|---|
...
| Địa chỉ nhà hàng + biển hiệu | **Domain & DNS** — `compassscribe.com` |
...
8. DNS + HTTPS   Cloudflare trỏ domain về máy chủ; ACM cấp khoá bảo mật
        │
9. GO LIVE       https://compassscribe.com chạy thật
        │
10. VẬN HÀNH     GitOps: mọi thay đổi qua Git, tự động & an toàn
```

## 📁 Sơ đồ kiến trúc thật của Compassscribe
...
```
   NGƯỜI DÙNG (trình duyệt điện thoại/máy tính)
        │  gõ compassscribe.com
        ▼
   ┌─────────────────┐
   │  CLOUDFLARE DNS │  "compassscribe.com ở đâu?" → trả về địa chỉ máy chủ
   └─────────────────┘
        │
   ...
   │        │  POST api.compassscribe.com/api/lead                           │
   │        ▼                                                                  │
   │   ┌──────────────┐    ┌──────────┐    ┌──────────────┐   ┌────────┐      │
...
minh "tôi đúng là compassscribe.com". Trong dự án bạn, chứng chỉ này do **AWS ACM** cấp miễn phí
(Chương 13).

## ⚙️ DNS — danh bạ của Internet

Con người nhớ tên (`compassscribe.com`), nhưng máy tính cần địa chỉ IP (`52.x.x.x`). **DNS** (Domain
Name System) là cuốn **danh bạ** dịch tên → địa chỉ.

```
   Bạn gõ:  compassscribe.com
        │
        ▼
   ┌─────────┐   "compassscribe.com có IP nào?"
   │   DNS   │ ◀──────────────────────────────────
...
## 📁 Trong dự án Compassscribe: một lần xem trang chủ

Khi một bác lớn tuổi mở `compassscribe.com` trên điện thoại, đây là chuỗi sự kiện thật:

```
1. Điện thoại hỏi DNS (Cloudflare): "compassscribe.com ở đâu?"
2. Cloudflare trả: "tới cmas-alb-....elb.amazonaws.com" (máy chủ ALB của bạn trên AWS)
...
| Tầng | Trách nhiệm | Chạy ở đâu | Trong Compassscribe |
|---|---|---|---|
...
| **Database** | Lưu trữ dữ liệu lâu dài | **Máy chủ database** | DynamoDB `cmas-leads` |
...
## 📁 Trong dự án Compassscribe: một lead đi qua ba tầng
...
Trong Compassscribe, mỗi phần của trang là một component:
```
...
Toàn bộ dự án + lịch sử của nó. Repo của bạn: `compass247/compassscribe` trên GitHub. Có 2 bản:
...
"Build Compassscribe production site", qua "full local backend stack", tới "GitOps: auto-apply
...
Trong Compassscribe, **toàn bộ chữ** (cả tiếng Việt và tiếng Anh) nằm trong **một file dữ liệu**:
...
- `VITE_API_BASE` — địa chỉ backend API.
  ...
  - Khi **build production**: đặt `https://api.compassscribe.com` → form gọi API thật.
...
- **Hard-code giá trị môi trường**: ghi chết `https://api.compassscribe.com` vào code → không chạy
...
## ⚙️ Các dịch vụ AWS dùng trong Compassscribe (giải thích từng cái)
...
- **IAM user**: `cmas-admin` (danh tính bạn dùng để điều khiển AWS từ máy local).
...
Lambda xử lý form chỉ được quyền `PutItem` (ghi) vào đúng bảng `cmas-leads`, không được xoá, không
đụng bảng khác. Nếu Lambda bị khai thác, thiệt hại bị giới hạn.

> Bạn đã gặp IAM thật khi: tạo user `cmas-admin`, và khi mở rộng quyền cho role deploy để CI tự
...
1. Vì sao thuê cloud thường tốt hơn tự mua máy chủ cho dự án như Compassscribe?
...
**Dockerfile** là file ghi các bước để tạo image. Dockerfile của Compassscribe dùng kỹ thuật **multi-stage**
...
- **Task**: một container đang chạy (một "nhân viên"). Trong Compassscribe: một bản nginx phục vụ web.
...
- **Cluster**: nhóm logic chứa các service (vd cluster `cmas`).
...
## ⚙️ Vì sao Compassscribe chọn DynamoDB?
...
Trong Compassscribe, khoá chính là `leadId` (một mã ngẫu nhiên duy nhất cho mỗi lead). Ngoài khoá chính,
...
ghi bằng lệnh `aws dynamodb scan --table-name cmas-leads` (đã dùng thật khi kiểm tra form).
...
1. Phân biệt SQL và NoSQL. Vì sao Compassscribe chọn DynamoDB cho lead?
...
kiện kích hoạt**. Trong Compassscribe, sự kiện là "có ai POST tới `/api/lead`". Mỗi lần như vậy, Lambda
`cmas-lead-handler` chạy một lần để xử lý lead đó.
...
```
   Form (frontend)
        │  POST https://api.compassscribe.com/api/lead
        ▼
   ┌──────────────┐      ┌──────────┐      ┌────────────┐
...
**mặc định chặn** một trang web ở domain này gọi API ở domain khác. Trang `compassscribe.com` gọi
`api.compassscribe.com` là "khác origin" → bị chặn, trừ khi API **cho phép rõ ràng**.

API Gateway của bạn được cấu hình CORS cho phép đúng origin `https://compassscribe.com`. Đây là lý
...
```
1. Frontend (src/api.js) gửi POST tới https://api.compassscribe.com/api/lead
...
3. Lambda (backend/lead-handler/index.mjs):
   ...
   • ghi vào DynamoDB (bảng cmas-leads)
...
Ví dụ: "tôi muốn có một bảng DynamoDB tên `cmas-leads` với khoá `leadId`". Terraform tự figure out
...
là "bộ nhớ" của Terraform. Dự án bạn lưu state trên **S3** (`cmas-tfstate-...`) + khoá chống ghi
...
compassscribe". AWS kiểm vé, cấp quyền tạm thời. **Không có mật khẩu nào được lưu** ở đâu cả.

```
   GitHub Actions          AWS
        │  "tôi là workflow của repo compass247/compassscribe" (vé OIDC ký số)
        │ ───────────────────▶
        │                     kiểm vé → đúng repo tin cậy?
        │  ◀─────────────────  cấp quyền tạm thời (role cmas-github-deploy)
        ▼
...
- **Variables** (biến): thông tin không nhạy cảm (vd `AWS_REGION = us-east-1`, `ECS_CLUSTER = cmas`).
...
- **Domain** (`compassscribe.com`) = cái tên dễ nhớ, như tên cửa hàng.
...
- **A record**: trỏ tên → một **địa chỉ IP cố định** (vd `compassscribe.com → 52.1.2.3`).
- **CNAME record**: trỏ tên → **một tên khác** (vd `compassscribe.com → cmas-alb-xxx.elb.amazonaws.com`).

## ⚙️ Vì sao Compassscribe phải dùng CNAME (không phải A)?
...
Giải pháp: dùng **CNAME** trỏ tới *tên* của ALB (`cmas-alb-xxx.elb.amazonaws.com`). Tên này cố
định; AWS lo việc tên đó luôn trỏ đúng IP hiện tại. Bạn "chỉ tên, không chỉ số".

```
   compassscribe.com ──CNAME──▶ cmas-alb-xxx.elb.amazonaws.com ──▶ (IP thật, AWS tự quản)
```

## ⚙️ HTTPS & chứng chỉ ACM

Để có ổ khoá xanh, server cần **chứng chỉ SSL/TLS** chứng minh danh tính. Dự án bạn dùng **AWS ACM**
(Certificate Manager) cấp chứng chỉ **miễn phí** cho `compassscribe.com` + `www` + `api`.
...
- `compassscribe.com` + `www` → CNAME tới ALB (xem website).
- `api.compassscribe.com` → CNAME tới API Gateway (gửi form).
...
Website (`compassscribe.com`) chạy trên ALB→ECS, còn API (`api.compassscribe.com`) chạy trên API
Gateway→Lambda — **hai hệ thống khác nhau**. Tách thành subdomain riêng cho phép mỗi cái trỏ tới đúng
nơi, độc lập. Frontend gọi `api.compassscribe.com` để gửi form, trong khi người dùng xem web ở domain
...
4. Vì sao API tách thành `api.compassscribe.com` riêng?
...
Cloud trả theo dùng, nên hiểu hoá đơn là kỹ năng quản lý quan trọng. Chi phí Compassscribe (~25-30 USD/tháng):
...
- **Logs**: nhật ký từ Lambda và ECS. Lệnh thật bạn dùng được:
  ```
  aws logs tail /aws/lambda/cmas-lead-handler --follow   # log Lambda (form)
  aws logs tail /ecs/cmas-web --follow                   # log web container
  ```
...
2. Chi phí chính của Compassscribe đến từ đâu (cố định hay theo traffic)?
...
| **Domain** | Tên miền (compassscribe.com) |
...
npm run db:init     # tạo bảng cmas-leads trong DynamoDB Local
...
aws ecs describe-services --cluster cmas \
    --services cmas-web --region us-east-1            # trạng thái web service
aws dynamodb scan --table-name cmas-leads \
...
terraform init -backend-config="bucket=cmas-tfstate-381492229787" \
...
> **Học trên chính dự án Compassscribe của bạn.** Mỗi khi đọc về một khái niệm, mở file/dịch vụ tương ứng
...
— *Tài liệu học nền tảng, dự án Compassscribe.*
